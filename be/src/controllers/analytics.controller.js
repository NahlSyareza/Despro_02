...
const getChartData = async (req, res) => {
  ...
  try {
    ...
    // 2. DATA NUTRITION QUALITY TREND (Grafik Hijau)
    // Rata-rata score kepatuhan per hari (7 hari terakhir)
    const nutritionTrendQuery = `
      SELECT TO_CHAR(date, 'Dy') as day_name, AVG(compliance_score) as avg_score
      FROM tray 
      WHERE vendor_id = $1 AND date >= CURRENT_DATE - INTERVAL '7 days'
      GROUP BY date
      ORDER BY date ASC
    `;
    ...
    const nutritionTrend = nutritionRes.rows.map(row => ({
      date: row.day_name.trim(), // Contoh: "Mon", "Tue"
      thisWeek: parseFloat(row.avg_score).toFixed(1),
      previousWeek: 0 // Placeholder jika belum ada komparasi minggu lalu
    }));

    // 3. DATA OVERALL RATING TREND (Grafik Ungu) -- BARU
    // Rata-rata rating review per hari (7 hari terakhir)
    const ratingTrendQuery = `
      SELECT TO_CHAR(date, 'Dy') as day_name, AVG(rating) as avg_rating
      FROM review 
      WHERE vendor_id = $1 AND date >= CURRENT_DATE - INTERVAL '7 days'
      GROUP BY date
      ORDER BY date ASC
